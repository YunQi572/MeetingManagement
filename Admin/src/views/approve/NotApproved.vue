...
export default {
    ...
    methods: {
        async getList () {
            const { data: res } = await this.$http.get('/application/history', { params: this.queryInfo })
            if (res.code !== 100) {
        ...
        async accept (id) {
            ...
            const { data: res } = await this.$http.put('/application/accept', { applicationId: id })
            if (res.code !== 100) {
        ...
        async rej () {
            ...
            this.$refs.rejFormRef.validate(async valid => {
                ...
                const { data: res } = await this.$http.put('/application/reject', this.rejForm)
                // console.log(res)
                if (res.code !== 100) {
