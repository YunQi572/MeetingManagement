...
export default {
    ...
    methods: {
        async getAcceptList () {
            const { data: res } = await this.$http.get('/application/history', { params: this.acceptInfo })
            if (res.code !== 100) {
        ...
        async getRejectList () {
            const { data: res } = await this.$http.get('/application/history', { params: this.rejectInfo })
            if (res.code !== 100) {
                return this.$message.error('获取申请列表失败！')
            } else {
