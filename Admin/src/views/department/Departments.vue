...
export default {
    ...
    methods: {
        async getDeptList () {
            const { data: res } = await this.$http.get('/department/findpage', { params: this.queryInfo })
            if (res.code !== 100) {
        ...
        async selectDept () {
            const { data: res } = await this.$http.get('/department/deptCondition', { params: { deptName: this.keyWord, currentPage: this.queryInfo.currentPage, pageSize: this.queryInfo.pageSize } })
            if (res.code !== 100) {
        ...
        async showEditDialog (id) {
            const { data: res } = await this.$http.get('/department/find/'+{deptId: id}, { params: { deptId: id } })
            if (res.code !== 100) {
                return this.$message.error('查询部门信息失败！')
            }
        ...
        async removeDeptById (id) {
            ...
            const { data: res } = await this.$http.delete('/department/delet', { data: { deptId: id } })
            if (res.code !== 100) {
