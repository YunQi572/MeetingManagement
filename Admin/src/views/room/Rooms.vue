...
export default {
    ...
    methods: {
        async getRoomList () {
            const { data: res } = await this.$http.get('/room/findpage', { params: this.queryInfo })
            if (res.code !== 100) {
        ...
        async airStatusChanged (roomInfo) {
            const { data: res } = await this.$http.put('/room/update', { roomId: roomInfo.roomId, air: roomInfo.air })
            if (res.code !== 100) {
                roomInfo.roomStatus = !roomInfo.roomStatus
        ...
        async projectorStatusChanged (roomInfo) {
            const { data: res } = await this.$http.put('/room/update', { roomId: roomInfo.roomId, projector: roomInfo.projector })
            if (res.code !== 100) {
                roomInfo.roomStatus = !roomInfo.roomStatus
        ...
        async selectRoom () {
            const { data: res } = await this.$http.get('/room/findNo', { params: { roomNo: this.keyWord, currentPage: this.queryInfo.currentPage, pageSize: this.queryInfo.pageSize } })
            if (res.code !== 100) {
        ...
        addRoom () {
            this.$refs.addFormRef.validate(async valid => {
                if (!valid) return
                const { data: res } = await this.$http.post('/room/add', { roomNo: this.addForm.roomNo, roomSize: this.addForm.roomSize - 0 })
                if (res.code !== 100) {
        ...
        async showEditDialog (id) {
            const { data: res } = await this.$http.get('/room/find/'+{roomId: id}, { params: { roomId: id } })
            console.log(res)
            if (res.code !== 100) {
                return this.$message.error('查询会议室信息失败！')
            }
        ...
        eidtRoom () {
            this.$refs.editFormRef.validate(async valid => {
                if (!valid) return
                const { data: res } = await this.$http.put('/room/update', { roomId: this.editForm.roomId, roomNo: this.editForm.roomNo, roomSize: this.editForm.roomSize - 0 })
                if (res.code !== 100) {
        ...
        async removeRoomById (id) {
            ...
            const { data: res } = await this.$http.delete('/room/delete', { data: { roomId: id } })
            if (res.code !== 100) {
